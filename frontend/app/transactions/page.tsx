...
export default function TransactionsPage() {
  ...
  const transactions = (cases || []).filter((c) => {
  ...
  const columns = [
    ...
    },
    {
      key: 'case_type',
      header: 'Type',
      render: (c: Case) => <span className="text-xs text-text-2">{formatCaseType(c.case_type)}</span>,
    },
    {
      key: 'amount_at_risk_minor',
      header: 'Amount',
      className: 'text-right',
      render: (c: Case) => <span className="font-semibold tabular text-text-1">{formatPaise(c.amount_at_risk_minor)}</span>,
    },
    {
      key: 'risk',
      header: 'Risk',
    ...
    },
    {
      ...
      header: 'Merchant',
      hideOnMobile: true,
      render: (c: Case) => <span className="font-mono text-xs text-text-3">{truncateId(c.merchant_id)}</span>,
    },
    {
      key: 'customer_id',
      header: 'Customer',
      hideOnMobile: true,
      render: (c: Case) => <span className="font-mono text-xs text-text-3">{truncateId(c.customer_id)}</span>,
    },
    {
      key: 'detected_at',
      header: 'Time',
      render: (c: Case) => <span className="text-xs text-text-3">{formatTimeAgo(c.detected_at)}</span>,
    },
  ];
  ...
  return (
    <div className="space-y-6 page-enter">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-text-1">Transactions</h1>
        <p className="mt-1 text-sm text-text-3">Failed payment transactions and their recovery status.</p>
      </div>
      ...
      <Card padding="none">
        {isError ? (
          ...
          <EmptyState icon={Zap} title="No transactions yet" description="Failed payment transactions will appear here." />
        ) : (
          <>
            <div className="border-b border-border px-5 py-2.5 text-xs text-text-3">{transactions.length} transaction{transactions.length !== 1 ? 's' : ''}</div>
            <DataTable columns={columns} data={transactions} keyExtractor={(c) => c.id} onRowClick={(c) => router.push(`/cases/${c.id}`)} />
          </>
        )}
      </Card>
    </div>
